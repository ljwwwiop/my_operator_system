...
KeyBoardDriver::KeyBoardDriver(InterruptManager* manager,KeyboardEventHandler* handler)
    :InterruptHandler(0x01 + manager->HardwareInterruptOffset(),manager),dataport(0x60),commandport(0x64),handler(handler)
{
...
void KeyBoardDriver::Activate()
{
    ...
    // 读取 8bit
    uint8_t status = (dataport.Read() | 1) & ~0x10; // | 1 是开启键盘中断，把键盘键盘开启 ，取反获取到第四位
