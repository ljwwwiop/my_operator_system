...
Widget::Widget(Widget* parent,int32_t x,int32_t y,int32_t w,int32_t h,uint8_t r,uint8_t g,::uint8_t b)
    :KeyboardEventHandler(),parent(parent),x(x),y(y),w(w),h(h),r(r),g(g),b(b),Focussable(true)
{
}
...
void Widget::Draw(myso::common::GraphicsContext* gc)
{
    int X = 0,Y = 0;
    ModelToScreen(X,Y);
...
void CompositeWidget::Draw(myso::common::GraphicsContext* gc)
{
    Widget::Draw(gc);
    // ~i = i>=0
