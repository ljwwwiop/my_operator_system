...
{
    if(size < sizeof(InternetControlMessaageProtocolMessage) ){
        return false;
    }

    InternetControlMessaageProtocolMessage* msg = (InternetControlMessaageProtocolMessage*)internetProtocolPayload;
    switch(msg->type){
    case 0:
    ...
    case 8:
        msg->type = 0;
        msg->checkSum = 0;
        msg->checkSum = InternetProtocolProvider::CheckSum((uint16_t*)msg, sizeof(InternetControlMessaageProtocolMessage));
        return true;
    }
    return false;
...
{
    InternetControlMessaageProtocolMessage icmp;
    icmp.type = 8;
    icmp.code = 0;
    icmp.data = 0x3713; // 0x1337
    icmp.checkSum = 0;
    icmp.checkSum = InternetProtocolProvider::CheckSum((uint16_t*)&icmp, sizeof(InternetControlMessaageProtocolMessage));
    InternetProtocolHandler::Send(ip_be,(uint8_t*)&icmp, sizeof(InternetControlMessaageProtocolMessage));
