...
InternetProtocolProvider::InternetProtocolProvider(EtherFrameProvider* backend,AddressResolutionProtocal* arp, uint32_t gatewayIP,uint32_t subnetMASK)
...
bool InternetProtocolProvider::OnEtherFrameReceived(uint8_t* buffer, uint32_t size)
{
    ...
    if(ipMessage->dstIP == backend->GetIPAddress()){
        ...
        if(handlers[ipMessage->protocol] != 0){
            sendBack = handlers[ipMessage->protocol]->OnInternetProtocolReceived(ipMessage->srcIP,ipMessage->dstIP,buffer + 4 * ipMessage->headerLength, length - 4 * ipMessage->headerLength);
        }
    }
...
void InternetProtocolProvider::Send(uint32_t dstIP_BE,uint8_t protocol,uint8_t* data,uint32_t size)
{
    ...
    message->tos = 0;
    message->totalLength = sizeof(InternetProtocolV4Message) + size;
